...
/**
 * @brief 响应主机发出的 GetStatus 请求，通过usb_device_respond_setup_request返回信息
 *
...
void usb_device_respond_get_status_request(nrf_drv_usbd_setup_t const * const p_setup)
{
    ...
    case 0x80: // 设备请求 
        ...
        {
            // 响应相应的设备状态
            usb_device_respond_setup_request(
                p_setup,	// 请求包
...
void usb_device_respond_get_descriptor_request(nrf_drv_usbd_setup_t const * const p_setup)
{
    ...
    case 0x21: // HID描述符请求
        if ((p_setup->bmRequestType) == 0x81) // 请求类型为 标准 设备到主机 接口 请求
        {
            ...
            if (((p_setup->wValue) & 0xFF) == 0) // 仅支持接口0
            {
                usb_device_respond_setup_request( // 发送HID接口描述符
                    p_setup,
    ...
    case 0x22: // HID报告描述符请求
        if ((p_setup->bmRequestType) == 0x81) // 请求类型为 标准 设备到主机 接口 请求
        {
            ...
            if (((p_setup->wValue) & 0xFF) == 0) // 仅支持接口0
            {
                usb_device_respond_setup_request( // 发送HID报告描述符
                    p_setup,
...
void usb_device_respond_get_config_request(nrf_drv_usbd_setup_t const * const p_setup)
{
    ...
    {
        usb_device_respond_setup_request( // 发送已配置的响应
            p_setup,
    ...
    {
        usb_device_respond_setup_request( // 发送未配置的响应
            p_setup,
...
void usb_device_respond_set_config_request(nrf_drv_usbd_setup_t const * const p_setup)
{
    if ((p_setup->bmRequestType) == 0x00) // 确保请求类型为 标准 主机到设备 设备 请求
    {
            ...
            ((p_setup->wValue) <= UINT8_MAX))
        {
            ...
            if (NRF_SUCCESS == usb_device_configure_endpoint((uint8_t)(p_setup->wValue),NRF_DRV_USBD_EPIN1))
            {
                nrf_drv_usbd_setup_clear(); // 接受了请求，清除设置阶段，处理完成
                return; // 处理成功，返回
            }
 ...
 * @param[in] p_setup 指向设置请求数据的指针。
 */
static void usb_device_respond_set_protocol_request(nrf_drv_usbd_setup_t const * const p_setup)
{
    if (p_setup->bmRequestType == 0x21) // 确保请求类型为 class 主机到设备 接口 请求
    {
        // 接受任何值
    ...
    case NRF_DRV_USBD_EVT_RESET: // 请求重置
        {
            ...
            ret_code_t ret = usb_device_configure_endpoint(0, NRF_DRV_USBD_EPIN1); // 重置端点1配置
    ...
    case NRF_DRV_USBD_EVT_SETUP: // 处理设置请求事件，主机请求设置从机/获取从机信息
        {
            ...
            {
            case 0x00: // GetStatus
                usb_device_respond_get_status_request(&setup); // 获取状态请求处理
                break;
            case 0x01: // ClearFeature
                usb_device_respond_clear_feature_request(&setup); // 清除特性请求处理
                break;
            case 0x03: // SetFeature
                usb_device_respond_set_feature_request(&setup); // 设置特性请求处理
                break;
            case 0x05: // SetAddress
                // 不做任何操作，由硬件处理，但不进入 STALL
                break;
            case 0x06: // GetDescriptor
                usb_device_respond_get_descriptor_request(&setup); // 获取描述符请求处理
                break;
            case 0x08: // GetConfig
                usb_device_respond_get_config_request(&setup); // 获取配置请求处理
                break;
            case 0x09: // SetConfig
                usb_device_respond_set_config_request(&setup); // 设置配置请求处理
                break;
            // HID 类
            case 0x0A: // SetIdle
                usb_device_respond_set_idle_request(&setup); // 设置空闲请求处理
                break;
            case 0x0B: // SetProtocol 或 SetInterface
                if (setup.bmRequestType == 0x01) // 标准请求，接收者为接口
                {
                    usb_device_respond_set_interface_request(&setup); // 设置接口请求处理
                }
                else if (setup.bmRequestType == 0x21) // 类请求，接收者为接口
                {
                    usb_device_respond_set_protocol_request(&setup); // 设置协议请求处理
                }
                else
                {
...
void usb_device_power_event_handler(nrf_drv_power_usb_evt_t event)
{
    ...
    case NRF_DRV_POWER_USB_EVT_REMOVED: // USB 电源被移除
        ...
        /* 关闭 LED */
        nrf_gpio_pin_set(LED_MOUSE_SUSPENDED); // 关闭 挂起 状态 LED
...
void usb_device_force_suspend(void)
{
		...
		nrf_gpio_pin_clear(LED_MOUSE_SUSPENDED); // 打开表示系统挂起状态的LED
}
...
void usb_device_force_leaving_suspend(void)
{
		...
		{
				nrf_gpio_pin_clear(LED_MOUSE_CONNECTED);
		}
		nrf_gpio_pin_set(LED_MOUSE_SUSPENDED); // 关闭表示系统挂起状态的LED
...
void usb_device_init(void)
{
    ...
    else
    {
        ...
        if (!nrf_drv_usbd_is_enabled())
        {
            ...
            ret = usb_device_configure_endpoint(0, NRF_DRV_USBD_EPIN1); // 配置端点 1
            APP_ERROR_CHECK(ret);
        }
