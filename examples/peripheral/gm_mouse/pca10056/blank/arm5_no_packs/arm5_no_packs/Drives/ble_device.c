    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            NRF_LOG_INFO("Device connected. Total connections: %d", connection_count);
						NRF_LOG_FLUSH();
            nrf_gpio_pin_clear(LED_MOUSE_CONNECTED); /**< 设置连接状态的LED指示。 */
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            NRF_LOG_INFO("Device disconnected. Total connections: %d", connection_count);
						NRF_LOG_FLUSH();
						nrf_gpio_pin_set(LED_MOUSE_CONNECTED); /**< 设置连接状态的LED指示。 */
						nrf_gpio_pin_set(LED_MOUSE_SUSPENDED); /**< 设置挂起状态的LED指示。 */
            // LED指示将在广播启动时更改。
...
void ble_device_advertising_init(void)
{
    ...
    // 配置广播模式。
    init.config.ble_adv_whitelist_enabled          = true;               /**< 禁用白名单广播模式。只有在白名单中的设备（即预先配对或允许的设备）才能扫描到该广播 */
...
void ble_device_set_filtered_device_identity_list(pm_peer_id_list_skip_t skip)
{
    ...
    if (peer_id_count == 0)
    {
        NRF_LOG_WARNING("No peers available to set in device identities list.");
        return;
    }
...
void ble_device_peer_manager_event_handler(pm_evt_t const * p_evt)
{
    ...
    switch (p_evt->evt_id)
    {
        ...
        case PM_EVT_PEERS_DELETE_SUCCEEDED:
						NRF_LOG_INFO("Restart Advtersing");
            NRF_LOG_FLUSH();
            ble_device_advertising_start(false, true); /**< 删除所有绑定数据成功后，重新启动广播。 */
            break;
        ...
        case PM_EVT_PEER_DATA_UPDATE_SUCCEEDED:
            ...
            {
                NRF_LOG_INFO("Bonding failed due to missing key. Re-pairing required. Please delete the device on host");
								NRF_LOG_FLUSH();
            }
            break;
        default:
...
void ble_device_generate_random_private_address(void)
{
    ...
    // 设置新地址
    ret_code_t err_code = sd_ble_gap_addr_set(&new_addr);
    APP_ERROR_CHECK(err_code);
...
{
		// 点亮系统LED和按钮
    nrf_gpio_pin_set(LED_MOUSE_CONNECTED); // 关闭 鼠标连接 LED
