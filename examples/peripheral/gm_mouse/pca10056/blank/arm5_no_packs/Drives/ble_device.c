...
static bool mark_device_disconnected = false;                                    /**< @brief 设备是否断联 */

static bool mark_need_host_delete = false;                                    /**< @brief 设备是否需要主机删除 */
...
/**@brief 获取 mark_device_disconnected 标志的函数。
 *
 * @return 当前 mark_device_disconnected 标志的布尔值。
 */
bool ble_device_get_mark_device_disconnected(void)
{
    return mark_device_disconnected;
}


/**@brief 设置 mark_device_disconnected 标志的函数。
 *
 */
void ble_device_set_mark_device_disconnected(bool is_disconncted)
{
    mark_device_disconnected = is_disconncted;
}

/**@brief 获取 mark_need_host_deleting 标志的函数。
 *
 * @return 当前 mark_need_host_deleting 标志的布尔值。
 */
bool ble_device_get_mark_need_host_delete(void)
{
    return mark_need_host_delete;
}


/**@brief 设置 mark_need_host_deleting 标志的函数。
 *
 */
void ble_device_set_mark_need_host_delete(bool need_delete)
{
    mark_need_host_delete = need_delete;
}
...
// LED指示灯模块
APP_TIMER_DEF(module_led_timer);  // 定义一个LED定时器
...
void ble_device_disconnect(void)
{
    ...
    {
        NRF_LOG_INFO("Attempting to disconnect, conn_handle: 0x%04X", module_connection_handle);

        // 调用断开连接函数
        ret_code_t err_code = sd_ble_gap_disconnect(
            module_connection_handle, /**< 当前连接句柄 */
            BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION /**< 断开原因：用户主动断开 */
        );

        if (err_code == NRF_SUCCESS || err_code == NRF_ERROR_INVALID_STATE)
        {
            NRF_LOG_INFO("Disconnection request sent successfully.");
						connection_count--;
        }
        else
        {
            // 记录错误并继续执行
            NRF_LOG_ERROR("Failed to disconnect. Error code: 0x%08X", err_code);
						APP_ERROR_CHECK(err_code); // 对非预期错误触发断点
        }
    }
    else
    {
        NRF_LOG_WARNING("No active connection to disconnect.");
    }
}
...
{
		app_timer_stop(module_led_timer);
}
...
void ble_device_observer_event_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            connection_count++;
            NRF_LOG_INFO("Device connected. Total connections: %d", connection_count);
						NRF_LOG_FLUSH();
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            NRF_LOG_INFO("Device disconnected. Total connections: %d", connection_count);
						NRF_LOG_FLUSH();
						
						// 设置标记
						ble_device_set_mark_device_disconnected(true);
        ...
        case BLE_GATTC_EVT_TIMEOUT:
            NRF_LOG_INFO("GATT Client Timeout."); /**< 日志信息：GATT客户端超时。 */
						NRF_LOG_FLUSH();
            // GATT客户端超时事件，触发断开连接。
        ...
        case BLE_GATTS_EVT_TIMEOUT:
            NRF_LOG_INFO("GATT Server Timeout."); /**< 日志信息：GATT服务器超时。 */
						NRF_LOG_FLUSH();
            // GATT服务器超时事件，触发断开连接。
...
void ble_device_advertising_init(void)
{
    ...
    // 配置广播模式。
    init.config.ble_adv_whitelist_enabled          = false;               /**< 禁用白名单广播模式。只有在白名单中的设备（即预先配对或允许的设备）才能扫描到该广播 */
    init.config.ble_adv_directed_high_duty_enabled = true;               /**< 启用高占空比定向广播，用于快速重连，此模式的广播频率很高，能够快速被上一个连接设备识别并建立连接 */
    init.config.ble_adv_directed_enabled           = true;              /**< 禁用普通定向广播，普通定向广播是一种较低频率的广播模式，广播目标是上一个连接的设备。 */
    init.config.ble_adv_directed_interval          = APP_ADV_DIRECT_INTERVAL;                  /**< 定向广播的间隔（若启用）。 */
    init.config.ble_adv_directed_timeout           = APP_ADV_DIRECT_DURATION;                  /**< 定向广播的超时时间（若启用）。 */
...
void ble_device_advertising_event_handler(ble_adv_evt_t ble_adv_evt)
{
    ...
    switch (ble_adv_evt)
    {
        case BLE_ADV_EVT_DIRECTED_HIGH_DUTY: /**< 高占空比定向广播事件，用于快速重连到上一个已连接的设备 */
            NRF_LOG_INFO("Directed High Duty advertising."); /**< 日志信息：定向广播。 */
						NRF_LOG_FLUSH();
						app_timer_stop(module_led_timer); // 停止计时器
						err_code = app_timer_start(module_led_timer, APP_TIMER_TICKS(500), NULL);
            APP_ERROR_CHECK(err_code);
            break;
				
				case BLE_ADV_EVT_DIRECTED: /**< 高占空比定向广播事件，用于快速重连到上一个已连接的设备 */
        ...
        case BLE_ADV_EVT_FAST:		/**< 快速广播事件，用于设备启动时吸引周围设备快速连接 */
            NRF_LOG_INFO("Fast advertising."); /**< 日志信息：快速广播。 */
						NRF_LOG_FLUSH();
						app_timer_stop(module_led_timer); // 停止计时器
						err_code = app_timer_start(module_led_timer, APP_TIMER_TICKS(1000), NULL);
            APP_ERROR_CHECK(err_code);
            break;
        ...
        case BLE_ADV_EVT_SLOW:	/**< 慢速广播事件，降低广播频率以节省功耗 */
            NRF_LOG_INFO("Slow advertising."); /**< 日志信息：慢速广播。 */
						NRF_LOG_FLUSH();
						app_timer_stop(module_led_timer); // 停止计时器
						err_code = app_timer_start(module_led_timer, APP_TIMER_TICKS(2000), NULL);
            APP_ERROR_CHECK(err_code);
#if SWIFT_PAIR_SUPPORTED == 1
        ...
        case BLE_ADV_EVT_WHITELIST_REQUEST: /**< 事件：广播请求白名单地址和 IRK（Identity Resolving Key）,使用过滤条件设置身份列表，排除没有 IRK 的设备。将更新的白名单列表应用于广播模块 */
				{
						...
						NRF_LOG_INFO("pm_whitelist_get returns %d addr in whitelist and %d irk whitelist", addr_cnt, irk_cnt); /**< 打印调试信息，输出白名单中地址和 IRK 的数量。 */
						NRF_LOG_FLUSH();
        ...
        case BLE_ADV_EVT_PEER_ADDR_REQUEST: /**< 事件：在广播请求绑定设备地址时，加载并设置已绑定设备的定向地址，使设备能够执行定向广播 */
				{
						...
						if (module_peer_device_id != PM_PEER_ID_INVALID) /**< 检查当前绑定设备 ID 是否有效。 */
						{
								...
								if (err_code != NRF_ERROR_NOT_FOUND) /**< 检查是否成功加载已绑定设备数据，确保数据存在。 */
								{
										...
										NRF_LOG_INFO("Peer address loaded: %x" , p_peer_addr->addr);
										NRF_LOG_INFO("Peer address type: %d", p_peer_addr->addr_type);
						
										ble_device_set_filtered_device_identity_list(PM_PEER_ID_LIST_ALL_ID); /**< 设置过滤条件，排除所有无中央地址解析的设备。 */
										
										// 将绑定设备地址传递给广播模块以执行定向广播
										err_code = ble_advertising_peer_addr_reply(&module_advertising, p_peer_addr);
										APP_ERROR_CHECK(err_code); /**< 检查广播模块地址设置是否成功，若出错则触发错误处理。 */
								}
								else
								{
										NRF_LOG_INFO("No valid peer data found for device ID: %d", module_peer_device_id);
								}
						}
						break;
				}
...
/**@brief 启动广播的函数。
 *
 * @details 该函数根据是否需要清除绑定信息和是否启用白名单来启动广播。
 *          如果 `erase_bonds` 为 true，则删除所有绑定信息，广播将在绑定删除成功的事件（PM_EVT_PEERS_DELETE_SUCCEEDED）中启动。
 *          如果 `use_whitelist` 为 true，则设置白名单以限制连接的设备，否则进行开放式广播。
 *
 * @param[in] erase_bonds   如果为 true，则先删除绑定信息再启动广播。
 * @param[in] use_whitelist 如果为 true，则启用白名单，否则禁用白名单。
 */
void ble_device_advertising_start(bool erase_bonds, bool random_address)
{
    ...
    if (erase_bonds == true)
    {
				...
				if (random_address)
				{
					mark_change_address = true;
				}
				else
				{
					mark_change_address = false;
				}
				
				// 删除所有绑定信息
        ble_device_delete_bonds(); 
    ...
    else
    {
        ...
        ble_device_set_filtered_white_list(PM_PEER_ID_LIST_ALL_ID); /**< 设置白名单以允许指定的设备连接。 */
				ble_device_set_filtered_device_identity_list(PM_PEER_ID_LIST_ALL_ID); /**< 设置设备身份以允许指定的设备连接。 */

        if (random_address)
			  {
						// 重设随机静态地址
			  		ble_device_generate_random_private_address();
					
				}
				
				// 启用广播
				if(ble_device_get_peer_count() > 0 && has_valid_peer() && has_device_identities())
				{
						ret = ble_advertising_start(&module_advertising, BLE_ADV_MODE_DIRECTED_HIGH_DUTY);
				}
				else
				{
						ret = ble_advertising_start(&module_advertising, BLE_ADV_MODE_FAST);
				}
			  ...
			  if (ret != NRF_SUCCESS)
			  {
			      NRF_LOG_ERROR("Advertising failed to start. Error: 0x%08X", ret);
						NRF_LOG_FLUSH();
			  }
    }
}
...
bool has_valid_peer(void)
{
    ...
    {
        NRF_LOG_ERROR("Failed to get peer ID list. Error: 0x%08X", err_code);
        return false;
    }

    NRF_LOG_INFO("Total peer IDs found: %d", peer_id_count);

    for (uint8_t i = 0; i < peer_id_count; i++)
    {
        pm_peer_data_bonding_t bonding_data;
        err_code = pm_peer_data_bonding_load(peer_ids[i], &bonding_data);
        if (err_code == NRF_SUCCESS)
        {
            ble_gap_addr_t *addr = &bonding_data.peer_ble_id.id_addr_info;
            NRF_LOG_INFO("Peer ID: %d", (int)i);
						NRF_LOG_INFO("Address: %02X:%02X:%02X:%02X:%02X:%02X",
             addr->addr[5], addr->addr[4], addr->addr[3],
             addr->addr[2], addr->addr[1], addr->addr[0]);
						NRF_LOG_INFO("Peer Type: %d", addr->addr_type);


            if (addr->addr_type == BLE_GAP_ADDR_TYPE_PUBLIC || addr->addr_type == BLE_GAP_ADDR_TYPE_RANDOM_STATIC)
            {
                NRF_LOG_INFO("Valid peer found: Peer ID[%d]", i);
                return true; // 存在有效地址
            }
        }
        else
        {
            NRF_LOG_WARNING("Failed to load bonding data for Peer ID[%d]. Error: 0x%08X", i, err_code);
        }
    }
...
{
    uint32_t identities_count = BLE_GAP_DEVICE_IDENTITIES_MAX_COUNT;

    // 获取设备身份列表
    ret_code_t err_code = sd_ble_gap_device_identities_set(NULL, NULL, identities_count);
    if (err_code != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("Failed to get device identities. Error: 0x%08X", err_code);
        return false;
    }

    NRF_LOG_INFO("Device identities count: %d", identities_count);

    if (identities_count > 0)
    {
        NRF_LOG_INFO("Valid device identities found.");
        return true; // 有有效设备身份
    }

    NRF_LOG_WARNING("No device identities found.");
    return false; // 无设备身份
}
 ...
 *
 * @details 蓝牙设备在连接过程中使用的身份信息，包括 蓝牙地址 和 Identity Resolving Key (IRK)，它用于标识蓝牙设备的唯一身份
 *
...
void ble_device_set_filtered_device_identity_list(pm_peer_id_list_skip_t skip)
{
    ...
    // 打印设备ID列表
    NRF_LOG_INFO("Total device identities: %d", peer_id_count);
    for (uint32_t i = 0; i < peer_id_count; i++)
    {
        NRF_LOG_INFO("Device Identity [%d]: ID=%d", i, peer_ids[i]);
    }

    // 检查是否有有效的 Peer ID
    if (peer_id_count == 0)
    {
        NRF_LOG_INFO("No peers available to set in device identities list.");
        return;
    }

    // 检查是否超出设备身份列表支持的最大数量
    if (peer_id_count > BLE_GAP_DEVICE_IDENTITIES_MAX_COUNT)
    {
        NRF_LOG_WARNING("Peer ID count exceeds the maximum allowed. Limiting to %d.", BLE_GAP_DEVICE_IDENTITIES_MAX_COUNT);
        peer_id_count = BLE_GAP_DEVICE_IDENTITIES_MAX_COUNT;  // 限制数量
    }
    ...
    if (err_code != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("Failed to set device identities list. Error: 0x%08X", err_code);
    }
    else
    {
        NRF_LOG_INFO("Device identities list successfully set.");
    }
}

/**@brief 设置过滤后的白名单的函数。
 *
 * @details Peer Manager 用于管理配对设备的唯一标识符
 *
 * @param[in] skip  过滤器参数，用于过滤Peer Manager中的设备ID列表。
 */
void ble_device_set_filtered_white_list(pm_peer_id_list_skip_t skip)
{
		// 用于存储获取到的设备 Peer ID 列表
    pm_peer_id_t peer_ids[BLE_GAP_WHITELIST_ADDR_MAX_COUNT]; 
		
		// 获取到的实际设备数量（符合条件的设备数量）
    uint32_t     peer_id_count = BLE_GAP_WHITELIST_ADDR_MAX_COUNT; 

		// PM_PEER_ID_INVALID : 指定不排除任何设备，获取所有设备
		// skip:过滤条件，决定哪些设备被跳过
    ...
    APP_ERROR_CHECK(err_code); /**< 检查设备ID列表获取是否成功，出错则触发错误处理。 */
		...
		// 打印设备ID列表
		NRF_LOG_INFO("Total peers: %d", peer_id_count);
		
		for (uint32_t i = 0; i < peer_id_count; i++)
		{
				NRF_LOG_INFO("Peer ID[%d]: %d", i, peer_ids[i]);
		}
		
		// 最大设备数量
    NRF_LOG_INFO("Max Whitelist Peers: %d", BLE_GAP_WHITELIST_ADDR_MAX_COUNT);
		NRF_LOG_FLUSH();
...
void ble_device_peer_manager_event_handler(pm_evt_t const * p_evt)
{
    ...
    switch (p_evt->evt_id)
    {
        ...
        case PM_EVT_PEERS_DELETE_SUCCEEDED:
            ...
            NRF_LOG_FLUSH();
						if(mark_change_address)
						{
							ble_device_advertising_start(false, true);
							mark_change_address = false;
						}
						else
						{
        ...
        case PM_EVT_PEER_DATA_UPDATE_SUCCEEDED:
            ...
            NRF_LOG_WARNING("Connection security failed, reason: %d", p_evt->params.conn_sec_failed.error);
						NRF_LOG_FLUSH();
            
            if (p_evt->params.conn_sec_failed.error == 133 || p_evt->params.conn_sec_failed.error == 136)
            {
								// 删除绑定
								ble_device_delete_bonds();
								ble_device_generate_random_private_address();
                NRF_LOG_INFO("Bonding failed: The host device does not match. To resolve, remove the saved pairing information from the host device and re-enter paring mode.");
								NRF_LOG_FLUSH();
								WS2812B_setHostDeleteLight();			
								mark_need_host_delete = true;
								mark_device_disconnected = true;							
...
void ble_device_delete_bonds(void)
{
    ...
    NRF_LOG_INFO("Erase bonds initiated."); /**< 日志信息：开始删除绑定信息。 */
    NRF_LOG_FLUSH();
	
		// 断开所有设备连接
		ble_device_disconnect();

    // 调用 Peer Manager 的接口删除所有绑定信息
    err_code = pm_peers_delete();
    if (err_code == NRF_SUCCESS)
    {
        NRF_LOG_INFO("Peer Manager: All bonds deleted successfully.");
    }
    else
    {
        NRF_LOG_ERROR("Failed to erase bonds. Error code: 0x%08X", err_code);
    }
    ...
    // 成功完成操作后刷新日志
    NRF_LOG_INFO("Erase bonds process completed.");
    NRF_LOG_FLUSH();
}
...
{
		// 停止广播（如果正在广播）
    ret_code_t err_code = sd_ble_gap_adv_stop(module_advertising.adv_handle); // 停止广播
    if (err_code == NRF_SUCCESS)
    {
    ...
    {
        // 如果广播未启动，忽略停止广播的错误
        NRF_LOG_INFO("Advertising was not running. No need to stop");
				NRF_LOG_FLUSH();
    }
		else
    {
...
void ble_device_generate_random_private_address(void)
{
    ...
    // 生成随机地址（最高两位需满足静态地址要求）
    uint8_t random_bytes[BLE_GAP_ADDR_LEN];
    sd_rand_application_vector_get(random_bytes, BLE_GAP_ADDR_LEN);
    random_bytes[5] |= 0xC0; // 设置最高两位为 11，符合随机静态地址要求

    memcpy(new_addr.addr, random_bytes, BLE_GAP_ADDR_LEN);

    // 设置新地址
    err_code = sd_ble_gap_addr_set(&new_addr);
    APP_ERROR_CHECK(err_code);

    NRF_LOG_INFO("Generated new random static address: %02X:%02X:%02X:%02X:%02X:%02X",
                 new_addr.addr[5], new_addr.addr[4], new_addr.addr[3],
                 new_addr.addr[2], new_addr.addr[1], new_addr.addr[0]);
		NRF_LOG_FLUSH();
}
...
void ble_device_timer_init(void)
{
    ret_code_t err_code;  /**< 用于存储函数返回的错误代码。 */
		...
		// 创建 LED 翻转定时器
    err_code = app_timer_create(&module_led_timer,
                                APP_TIMER_MODE_REPEATED,
                                ble_device_led_timer_handler);  // 设置回调函数
    APP_ERROR_CHECK(err_code);
}
...
void ble_device_init(void)
{
    ...
    WS2812B_clearLight();
		...
		// 定时器模块初始化
		ble_device_timer_init();              /**< 初始化定时器模块，为系统定时事件提供支持。 */
